...
template<typename TSrc, typename TRes, typename UDist,    const CBool needCheck = false,  std::enable_if_t<std::is_base_of<UDistance<TSrc, TRes>, UDist>::value, int> = 0>
class UDistanceCalculator : public UDistanceObject {
public:
    // 计算距离信息
    CStatus calculate(const TSrc* v1, const TSrc* v2,  CSize dim1, CSize dim2,  TRes& result,  CVoidPtr ext = nullptr) {
    ...
    // 计算距离信息
    CStatus calculate(const std::vector<TSrc>& v1, const std::vector<TSrc>& v2,  TRes& result, CVoidPtr ext = nullptr) {
        return calculate(v1.data(), v2.data(), v1.size(), v2.size(), result, ext);
    }

    // 批量计算距离信息
    CStatus calculate(const std::vector<TSrc>& query,const std::vector<std::vector<TSrc> >& nodes, std::vector<TRes>& results, CVoidPtr ext = nullptr) {
        CGRAPH_FUNCTION_BEGIN
        results.clear();
