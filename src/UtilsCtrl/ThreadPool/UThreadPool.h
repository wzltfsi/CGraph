...
public:
    // 通过默认设置参数，来创建线程池 , autoInit 是否自动开启线程池功能
    explicit UThreadPool(CBool autoInit = true,    const UThreadPoolConfig& config = UThreadPoolConfig()) noexcept;

    ~UThreadPool() override;
 
    // 设置线程池相关配置信息，需要在init()函数调用前，完成设置 , 通过单例类(UThreadPoolSingleton)开启线程池，则线程池默认init。需要 destroy 后才可以设置参数
    ...
    auto commit(const FunctionType& func, CIndex index = CGRAPH_DEFAULT_TASK_STRATEGY)-> std::future<typename std::result_of<FunctionType()>::type>;

    //  根据优先级，执行任务 , 建议，priority 范围在 [-100, 100] 之间
    template<typename FunctionType>
    auto commitWithPriority(const FunctionType& func,    int priority)-> std::future<typename std::result_of<FunctionType()>::type>;
