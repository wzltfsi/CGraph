...
public:
    ...
    template<typename T, CLevel level = CGRAPH_DEFAULT_ELEMENT_LEVEL,    std::enable_if_t<std::is_base_of<GNode, T>::value, int> = 0>
    GNodePtr createGNode(const GNodeInfo &info);

    // 根据传入的信息，创建Group信息
    template<typename T, CLevel level = CGRAPH_DEFAULT_ELEMENT_LEVEL, std::enable_if_t<std::is_base_of<GGroup, T>::value, int> = 0>
    ...
    // 在图中注册一个Element信息 ,  如果注册的是GNode信息，则内部自动生成 ,  如果注册的是GGroup信息，则需外部提前生成，然后注册进来
    template<typename T, CLevel level = CGRAPH_DEFAULT_ELEMENT_LEVEL, std::enable_if_t<std::is_base_of<GElement, T>::value, int> = 0>
    ...
    // 添加切面
    template<typename TAspect, typename TParam = GAspectDefaultParam,   std::enable_if_t<std::is_base_of<GAspect, TAspect>::value, int> = 0, std::enable_if_t<std::is_base_of<GAspectParam, TParam>::value, int> = 0>
    GPipeline* addGAspect(const GElementPtrSet& elements = std::initializer_list<GElementPtr>(),   TParam* param = nullptr);

    // 添加守护
    template<typename TDaemon, typename DParam = GDaemonDefaultParam,  std::enable_if_t<std::is_base_of<GDaemon, TDaemon>::value, int> = 0,  std::enable_if_t<std::is_base_of<GDaemonParam, DParam>::value, int> = 0>
